...
parser.add_argument(
    "--clean-torch", action="store_true", help="Clean CIF PyTorch data .json files"
)
...
def main():
    ...
    # load data
    dataset = CIFData(*args.data_options, disable_save_torch=args.disable_save_torch)
    ...
    validate(
        val_loader, model, criterion, normalizer, test=True, csv_name="val_results.csv"
    )
...
def validate(
    val_loader, model, criterion, normalizer, test=False, csv_name="test_results.csv"
):
    batch_time = AverageMeter()
    losses = AverageMeter()
    if args.task == "regression":
...
def save_checkpoint(
    state, is_best, filename=os.path.join("output", "checkpoint.pth.tar")
):
